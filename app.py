...
@app.route('/undersamp/<url>')
def undersamp(url):

    if url == 'latih':

        data1 = Hasil.query.filter((Hasil.status == 'latih') & (
            Hasil.label == 1) & (Hasil.is_active == 1))
        data0 = Hasil.query.filter((Hasil.status == 'latih') & (
            Hasil.label == 0) & (Hasil.is_active == 1))

        data = []
        if data1.count() > data0.count():
            jml = data1.count() - data0.count()
            for data1 in data1:
            ...
            temp = []
            while i <= jml:
                id_subject = random.choice(data)

                if id_subject not in temp:
        ...
        elif data0.count() > data1.count():
            jml = data0.count() - data1.count()
            for data0 in data0:
        ...
        data1 = Hasil.query.filter((Hasil.status == 'uji') & (
            Hasil.label == 1) & (Hasil.is_active == 1))
        data0 = Hasil.query.filter((Hasil.status == 'uji') & (
            Hasil.label == 0) & (Hasil.is_active == 1))
...
@app.route('/oversamp/<url>')
def oversamp(url):

    if url == 'latih':

        data1 = Hasil.query.filter((Hasil.status == 'latih') & (
            Hasil.label == 1) & (Hasil.over == 0))
        data0 = Hasil.query.filter((Hasil.status == 'latih') & (
            Hasil.label == 0) & (Hasil.over == 0))
...
@app.route('/reset-undersamp/<url>')
def Reset_undersamp(url):

    if url == 'latih':

        data = Hasil.query.filter(
            (Hasil.status == 'latih') & (Hasil.is_active == 0) | (Hasil.over == 1))

        for data in data:
            my_data = Hasil.query.get(data.id)
            my_data.is_active = 1
            my_data.over = 0
        ...
        data = Hasil.query.filter(
            (Hasil.status == 'uji') & (Hasil.is_active == 0) | (Hasil.over == 1))
...
def Index():
    ...
    # return render_template('coba.html', data=jml_stroke)
...
def shw_uji_subject():
    # data = Akurasi_subject.query.all()
...
def proses_uji_subject_by_id(id_subject, nilai_k):
    ...
    # temp_subject_uji = [
    #     [my_data.meanrr, my_data.sdrr, my_data.cvrr, my_data.rmssd]]
...
def uji_subject():
    ...
    # return render_template('coba.html', data=tmp)
...
def Akurasi():
    ...
    targetU = []
    for i in dUji:
        # dataU.append([i.meanrr, i.sdrr, i.cvrr, i.rmssd])
...
def exportexcel():
    ...
    # return render_template('coba.html', data=df)
